...
int main(int argc, char *argv[]) {
  int master_socket, client_size, message_size, rv;
  int yes = 1;
    ...
    for (int i = 0; i <= fdmax; ++i) {
      if (FD_ISSET(i, &temp_set)) {
        if (i == master_socket) {
          ...
          new = accept (master_socket, (struct sockaddr *) &client_addr,
          ...
          if((message_size = read(i, buf, BUFSIZE - 1)) < 0)
              error("Error reading from socket\n");
          printf("Buf: %d\n", buf[0]);
          if (message_size <= 0) {
            empty_message(i, &master_set);
            continue;
          }
